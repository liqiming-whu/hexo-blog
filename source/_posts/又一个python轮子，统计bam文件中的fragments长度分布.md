...
-m筛选方式。每个基因的区域会有多个转录本，为了避免一个fragment在同一个基因上按不同转录本统计多次，所以必须对转录本进行筛选，每个基因保留一个转录本。有4种保留方式`[first|last|max_len|min_len] `。分别是注释中的第一条，最后一条，最长和最短。

-b输出筛选后的bed12，可选。

-f输出筛选后的gtf，可选。支持gzip压缩格式，后缀加“.gz"就行了。

-o输出文件，可选。不设置这个参数，就会打印到标准输出。
